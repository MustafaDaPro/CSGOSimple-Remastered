...
void Visuals::RecoilCrosshair()
{
	...
	g_VGuiSurface->DrawSetColor(Color(0, 135, 255));
	...
	g_VGuiSurface->DrawLine(x - 8, y, x + 8, y);
	g_VGuiSurface->DrawLine(x - 8, y, x + 8, y);
...
void Visuals::AddToDrawList()
{
	for (auto i = 1; i <= g_EntityList->GetHighestEntityIndex(); ++i)
	{
		...
		if (g_Options.esp_dropped_weapons && entity->IsWeapon()) RenderWeapon(static_cast<C_BaseCombatWeapon*>(entity));
		if (g_Options.esp_dropped_weapons && entity->IsDefuseKit()) RenderDefuseKit(entity);
		if (entity->IsPlantedC4() && g_Options.esp_planted_c4) RenderPlantedC4(entity);
